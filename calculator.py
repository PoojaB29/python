...
class app(Frame):
    def __init__(self):
        ...
        display=StringVar()
        Entry(self,relief=RIDGE,textvariable=display,justify='right',bd="powder blue").pack(side=TOP,expand=Yes,fill=BOTH)
        ...
        for iEquals in "=":
            if iEquals=='=':
                ...
                btniEquals.bind('<ButtonRelease-1>',lambda e,s=self,storeObj=display:s.calc(stroeObj),'+')
            else:
